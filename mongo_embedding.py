...
ATLAS_VECTOR_SEARCH_INDEX_NAME = "FrameSemanticSearch"
...
class EmbeddingPredictionClient:
  ...
  def __init__(self, project : str,
    ...
    self.mongodb_client = pymongo.MongoClient(os.environ["MONGO_URI"])
    self.collection = self.mongodb_client.video_embeddings.frames
  ...
  def load_video(self, video_file):
    ...
    for idx, frame in tqdm(enumerate(iio.imiter(video_file))):
      ...
      if result == None:
        ...
        #     ids=[f"{idx:03d}"]
        # )
        requests.append(InsertOne({"_id": id, "embedding": response.image_embedding, "seek_time": idx / fps, "image_bytes": image_contents, "video_file": video_file}))
...
def main(argv):
  ...
  matches = client.collection.aggregate([
    {
        '$search': {
  ...
  print([t['seek_time'] for t in matches])
